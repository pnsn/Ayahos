...
from ayahos.wyrms import RingWyrm, BufferWyrm, PickWyrm, OutputWyrm
from ayahos.core.pick import Trigger
...
# Initialize  bufferwyrms
# Waveform buffer - use interpolation for overlapping samples
wavebuffwyrm = BufferWyrm(max_length=120, method=1)
...
# Prediction Buffer - use mean stacking for overlapping samples
predbuffwyrm = BufferWyrm(max_length=60, method=3)

# Pick Wyrm - use max value pick times
pickwyrm = PickWyrm(
    module_id=module_id,
    installation_id=installation_id,
    leading_mute=900)
# Output Wyrm
pickoutwyrm = OutputWyrm(
    pclass=Trigger,
    oclass=str,
...
ayahos.update({'InRing': iringwyrm,
               'WaveBuffer': wavebuffwyrm,
               'SBMTube': sbmtubewyrm,
               'PredBuffer': predbuffwyrm,
               'Pick': pickwyrm,
               'PickMsg': pickoutwyrm,
               'OutRing': oringwyrm})
