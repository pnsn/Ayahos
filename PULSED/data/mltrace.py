...
Classes
-------
:class:`~PULSED.data.mltrace.MLTrace`
:class:`~PULSED.data.mltrace.Buffer`
"""
...
from PULSED.util.seisbench import pretrained_dict
from PULSED.util.stats import estimate_quantiles, estimate_moments
...
def read_mltrace(data_file, **obspy_read_kwargs):
    ...
    of :meth:`~PULSED.data.mltrace.MLTrace.write` that are associated as
    ...
    :: OUTPUT ::
    :return mlt: [PULSED.data.mltrace.mltrace.MLTrace] mltrace object
...
class MLStats(Stats):
    """An ObsPy :class:`~obspy.core.mltrace.Stats` child class that
    has modified and extended default values relative to ObsPy's Stats
    to encapsulate additional metadata associated with the EWFlow
    :class:`~PULSED.data.mltrace.MLTrace` class.
    ...
    :type header: dict
    :return: self
    :rtype: PULSED.data.mltrace.MLStats
    """
    ...
    def __init__(self, header={}):
        """Create a :class: `~PULSED.data.mltrace.MLStats object
...
def _add_processing_info(func, *args, **kwargs):
    ...
    in favor of the :class:`~PULSED.data.mlstream.MLStreamStats`-based tracking of 
    processing steps.
    """
    return
    # """
    ...
    # Attribution: Directly adapted from the obspy.core.mltrace function of the same name.
    # """
...
class MLTrace(Trace):
    """An ObsPy :class:`~obspy.core.mltrace.MLTrace` child class that adds a `fold`
    ...
    `fold` conveys observation density/importance for sample(s) (added EWFlow syntax)
    ...
    .__add__     - aliases to the .merge() method that is similar to, albiet with fewer options,
                    to the obspy.core.mltrace.MLTrace.__add__ method
    ...
    def __init__(self, data=np.array([], dtype=np.float32), fold=None, header=None):
        ...
        :type header: dict or NoneType, optional
        :return: self
        :rtype: PULSED.data.mltrace.MLTrace
    ...
    def view_copy(self, starttime=None, endtime=None, pad=False, fill_value=None):
        ...
        as a view and then coppied into a new :class: `~PULSED.data.mltrace.MLTrace`
        object.
        ...
        :type fill_value: NoneType, int, float
            also see :meth: `~obspy.core.mltrace.MLTrace.trim`
                     :meth: `~PULSED.data.mltrace.MLTrace.trim`
        :return: 
            - **mlt** (*PULSED.data.mltrace.MLTrace*) -- new MLTrace conataining copied data (and fold) information
        ...
        if pad:
            ...
            # NTS (18 JUN 2024) Debugged with :meth:`~PULSED.data.mltrace.MLTrace._internal_add_processing_info`
        return mlt
    ...
    def get_fvalid_subset(self, starttime=None, endtime=None, threshold=1):
        ...
        :type endtime: obspy.core.utcdatetime.UTCDateTime, optional
            also see :meth: `~PULSED.data.mltrace.MLTrace.get_subset_view`
    ...
    def get_fold_trace(self):
        """
        Return an :class: `~obspy.core.mltrace.MLTrace` object with data=fold vector for this MLTrace,
        ...
        :rtype ft: obspy.core.mltrace.MLTrace
    ...
    def to_zero(self, method='both'):
        ...
        :meth: `~PULSED.data.windowstream.WindowStream.apply_fill_rule`
    ...
    def enforce_zero_mask(self):
        ...
        ..rubric: Toy Example for a :class: `~PULSED.data.mltrace.MLTrace` object 
    ...
    def __repr__(self, id_length=None):
        ...
        :class: `~PULSED.data.mltrace.MLTrace` object

        :param id_length: opional maximum ID length passed to the inherited
            :meth: `~obspy.core.mltrace.MLTrace.__str__` method, defaults to None
    ...
    def __eq__(self, other):
        ...
        :type other: any, seeking PULSED.data.mltrace.MLTrace
    ...
    def __add__(self, trace, method=1, fill_value=None, sanity_checks=True, reset_processing=True, dtype=np.float32):
        ...
        method 0 - conforms to obspy.core.mltrace.MLTrace.__add__(method=0) behavior
        ...
        method 1 - conforms to obspy.core.mltrace.MLTrace.__add__(method=1, interpolation_samples=-1)
        ...
        :type trace: obspy.core.mltrace.MLTrace or child class thereof
        ...
        :param fill_value: value to fill gaps with, defaults to None
                        also see :meth: `~obspy.core.mltrace.MLTrace.__add__`
                                 :meth: `~obspy.core.mltrace.MLTrace.trim`
                                 :meth: `~PULSED.data.mltrace.MLTrace.view_copy`
        ...
        # Discard Method (effectively obspy.core.mltrace.MLTrace.__add__(method=0))
        ...
        # Interpolation method (effectively the same as obspy.core.mltrace.MLTrace.__add__(method=1))
    ...
    def _relative_indexing(self, other):
        ...
        :type other: obspy.core.mltrace.MLTrace-like
    ...
    def _ltrim(self, starttime, pad=False, nearest_sample=True, fill_value=None):
        """
        Run the obspy.core.mltrace.MLTrace._ltrim() method on this MLTrace and trim
        ...
        of the fold array if starttime < self.stats.endtime and pad = True
         ...
         see :meth: `~obspy.core.mltrace.MLTrace._ltrim` for nearest_sample
         and pad behaviors
    ...
    def _rtrim(self, endtime, pad=False, nearest_sample=True, fill_value=None):
        """
        Run the obspy.core.mltrace.MLTrace._rtrim() method on this MLTrace and trim
        ...
        of the fold array if endtime > self.stats.endtime and pad = True
         ...
         see :meth: `~obspy.core.mltrace.MLTrace._rtrim` for nearest_sample
         and pad behaviors
    ...
    def split(self):
        """
        Slight modification to the obspy.core.mltrace.MLTrace.split() method
        wherein a Stream of MLTrace objects are returned, which includes 
        a trimmed version of the MLTrace.fold attribute.

        also see :meth:`~obspy.core.mltrace.MLTrace.split`
    ...
    def normalize(self, norm_type='max'):
        ...
        Extension of the obspy.core.mltrace.MLTrace.normalize method
    ...
    def treat_gaps(self,
        ...
        :param filterkw: keyword arguments to pass to :meth:`~PULSED.data.mltrace.MLTrace.filter`
            also see :meth:`~obspy.core.mltrace.MLTrace.filter
        :type filterkw: dict or bool
        :param detrendkw: keyword arguments to pass to :meth:`~PULSED.data.mltrace.MLTrace.detrend`,
        :type detrendkw: dict or bool
        :param resample_method: name of the resampling method to use. Supported values
                                    - 'resample' -- :meth:`~obspy.core.mltrace.MLTrace.resample`
                                    - 'interpolate' -- :meth:`~obspy.core.mltrace.MLTrace.interpolate`
                                    - 'decimate' -- :meth:`~obspy.core.mltrace.MLTrace.decimate`
        ...
        :param taperkw: keyword arguments to pass to :meth:`~PULSED.data.mltrace.MLTrace.taper`
                    also see :meth:`~obspy.core.mltrace.MLTrace.filter
        :type taperkw: dict or bool
        :param mergekw: keyword arguments to pass to :meth:`~PULSED.data.mltrace.MLTrace.merge`
                    also see :meth:`~obspy.core.mltrace.MLTrace.filter
        :type mergekw: dict or bool
        :param trimkw: keyword arguments to pass to :meth: `~PULSED.data.mltrace.MLTrace.trim`
                    also see :meth:`~obspy.core.mltrace.MLTrace.filter
    ...
    def resample(self, sampling_rate, window='hann', no_filter=True, strict_length=False):
        """
        Run the :meth:`~obspy.core.mltrace.MLTrace.resample` method on this MLTrace, interpolating
    ...
    def interpolate(
        ...
        """
        Run the :meth:`~obspy.core.mltrace.MLTrace.interpolate` method on this MLTrace, interpolating
        ...
        see :meth`~obspy.core.mltrace.MLTrace.interpolate` for detailed descriptions of inputs
        ...
        :param *args: dditional positional arguments to pass to the interpolator in :meth `~obspy.core.mltrace.MLTrace.interpolate`
        :type *args: list-like
        :param **kwargs: additional key word arguments to pass to the interpolator in :meth: `~obspy.core.mltrace.MLTrace.interpolate`
        :type **kwargs: kwargs
    ...
    def sync_to_window(self, starttime=None, endtime=None, fill_value=None, pad_after=True, **kwargs):
        ...
        :rtype: PULSED.data.mltrace.mltrace.MLTrace
    ...
    def decimate(self, factor, no_filter=False, strict_length=False):
        """
        Run the :meth:`~obspy.core.mltrace.MLTrace.decimate` method on this MLTrace, interpolating
    ...
    def to_trace(self, fold_threshold=0, attach_mod_to_loc=False):
        """
        Convert this MLTrace into a :class:`~obspy.core.mltrace.MLTrace` object,
        ...
        :returns:
            - **tr** (*obspy.core.mltrace.MLTrace*) -- trace object with data masked on samples with fold < fold_threshold
    ...
    def read(file_name):
        """
        Read a MSEED file that was generated by :meth:`~PULSED.data.mltrace.MLTrace.write` to reconstitute a new MLTrace object from
    ...
    def _prep_fold_for_mseed(self):
        ...
        :rtype fold_trace: obspy.core.mltrace.MLTrace
        """        
    ...
    def get_valid_fraction(self, thresh=1):
        ...
        compare to :meth: `~PULSED.data.mltrace.MLTrace.get_fvalid_subset`
    ...
    def _internal_add_processing_info(self, info):
        """
        Disable processing update via decorator from :class:`~obspy.core.trace.Trace`
        """
        pass