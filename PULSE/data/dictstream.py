...
:purpose:
    ...
    :class:`~PULSE.data.mltrace.MLTrace` class. It uses a dictionary object to hold traces,
    rather than the list used by :class:`~obspy.core.stream.Stream` to accelerate sorting, slicing,
    and querying individual traces (or sets of traces) via the hash-table functionality that 
    underlies python dictionaries. This places greater restriction on the objects that can be placed
    in a :class:`~PULSE.data.dictstream.DictStream` object in comparison to it's ObsPy forebearer
