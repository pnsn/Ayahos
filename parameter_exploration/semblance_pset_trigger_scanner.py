...
def semblance_trigger_site_event(evid, site):
    ...
    for label in ['P','S']:
        ...
        mysets = [MODSET, MODSET[:-1], MODSET[:-2]]
        my_id_sets = []
        for mset in mysets:
            my_id_sets.append(tuple([f'{site}.*.*.{model}.{weight}' for model, weight in mset]))
        mysets_match_str = [f'*.*.*.*.']
        for iset in mysets:
            sldst = dst.isin(iset)
...
if __name__ == '__main__':
    ...
    if run_parallel:
        # breakpoint()
        tick = time.time()
        print(f'==== LAUNCHING POOL FOR {len(evid_site_sets)} JOBS BY {n_pool} WORKERS ====')
        with mp.Pool(n_pool, initializer=init_worker, initargs=initargs) as p:
            sout = p.starmap(semblance_trigger_site_event, evid_site_sets, chunksize=1000)
        tock = time.time()
