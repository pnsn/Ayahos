...
class Trigger(FoldTrace):
    ...
    def __init__(self, source_trace, samp_on, samp_off, thr_on, thr_off, samp_pad=None):
        ...
        tts = source_trace.stats.starttime + \
            (self.samp_on - self.samp_padles)*source_trace.stats.delta
        tte = source_trace.stats.starttime + \
            (self.samp_off + self.samp_padles)*source_trace.stats.delta
        ...
        self.samp_off = self.count() - self.samp_pad
    ...
    # def fit_geometric_model(self, mindata=30.):
    #     """Under the assumption that the **data** in this :class:`~.Trigger`
    #     object resemble a Gaussian probability density function,
    #     fit a scaled Gaussian model:

    #     :math:`A e^{-\\frac{(x - E[x])^2}{2\sigma^2}}`

    #     to **data**

    #     :param mindata: _description_, defaults to 30.
    #     :type mindata: _type_, optional
    ...
    #     p0 = [self.pmax, self.xmax, (0.25*(self.samp_off - self.samp_on))**2]
    #     # Fit normal distribution
    #     pout, pcov, res = fit_normal_pdf_curve(xv, yv, threshold=0., mindata=mindata, p0=p0)
    #     if not hasattr(self.measures, 'gau_mod'):
    #         self.measures.gau_mod = {}
    #     self.measures.gau_mod.update(dict(zip(['m','cov','res'], [pout, pcov, res])))
