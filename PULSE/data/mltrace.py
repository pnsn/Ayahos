...
Classes
-------
:class:`~PULSE.data.mltrace.MLStats`
...
# TODO: Move to obsolescense - unused
@decorator
...
class MLTrace(Trace):
    ...
    :: ADDITIONAL METHODS ::
    .merge - provides methods 0) discard overlapping values (follows ObsPy behaviors)
                              1) linear interpolation across data overlaps (follows ObsPy behaviors)
                              2) max value of overlapping data samples (follows SeisBench behaviors for 'max' stacking)
                              3) fold-weighted average value of overlapping data samples (follows SeisBench behaviors for 'avg' stacking)
    ...
    def view_copy(self, starttime=None, endtime=None, pad=False, fill_value=None):
        """
        Create a trimmed copy of this MLTrace where the subset data are fetched as a view and then copied into a new :class:`~PULSE.data.mltrace.MLTrace` object.
