...
#     """
#     Wrapper around :meth:`~PULSE
# .data.mltrace.MLTrace.read_mltrace`
...
#                     PULSE
# .data.MLStream.MLStream.__add__

#     :: OUTPUT ::
#     :return dst: [PULSE
# .data.MLStream.MLStream] assembled MLStream object
...
class MLStreamStats(AttribDict):
    """
    A class to contain metadata for a PULSE
    .data.MLStream.MLStream object
...
class MLStream(Stream):
    """
    Dictionary like object of multiple EWFlow :class: `~PULSE
    .data.mltrace.MLTrace` objects
    ...
    :type traces: list of :class: `~obspy.core.trace.Trace` (or children) objects, optional
        NOTE: all initial traces are converted into :class: `~PULSE
        .data.mltrace.MLTrace` objects
    :param header: initial dictionary of key-value pairs to pass to EWFlow
        :meth: `~PULSE
        .data.mlstream.MLStreamStats.__init__`, defaults to {}
    ...
    :type key_attr: str, optional
        Based on N.S.L.C.M.W from EWFlow :class: `~PULSE
        .data.mltrace.MLTraceStats`
        ...
        Supported values:
            'id' - N.S.L.C(.M.W) code (see :class: `~PULSE
            .data.mltrace.MLTrace`)
            'site' - N.S code
        ...
        also see :meth: `~PULSE
        .data.mltrace.MLTrace.get_id_element_dict`
    :param **options: key word argument gatherer that passes optional kwargs to
        :meth: `~PULSE
        .data.mlstream.MLStream.__add__` for merging input
    ...
    def __add__(self, other, **options):
        ...
        into a list and initialize a new :class: `~PULSE
        .data.mlstream.MLStream`
        object.
    ...
    #         **obspy_write_options):
    #     """
    #     Wrapper around the PULSE
    # .data.mltrace.MLTrace.write() method
    #     that saves data as 
    ...
    def read(mltrace_mseed_files):
        ...
        also see PULSE
        .data.mltrace.MLTrace.write()
                 PULSE
                 .data.mltrace.MLTrace.read()
        """
    ...
    def isin(self, iterable, key_attr=None, ascopy=False):
        ...
        :: OUTPUT ::
        :return out: [PULSE
        .data.MLStream.MLStream] subset view/copy
        """
        out = self.__class__(header=self.stats.copy(), key_attr = self.default_key_attr)
        matches = []
        for _e in iterable:
    ...
    def normalize_traces(self, norm_type ='peak'):
        ...
        norm_type and :meth: `~PULSE
        .data.mltrace.MLTrace.normalize`
    ...
    def prediction_trigger_report(self, thresh, exclude_list=None, **kwargs):
        """Wrapper around the :meth: `~PULSE
        .data.mltrace.MLTrace.prediction_trigger_report`
        ...
        :type thresh: float
        :param exclude_list: :meth: `~PULSE
        .data.mlstream.MLStream.notin` compliant strings
                        to exclude from trigger processing, defaults to None
        :type exclude_list: list of str, optional
        :param **kwargs: gatherer for key word arguments to pass to :meth: `~PULSE
        .data.mltrace.MLTrace.prediction_trigger_report`
