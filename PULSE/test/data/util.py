...
def load_townsend_example():
    data = os.path.join('..','files','uw61965081_PortTownsend_M4.3.mseed')
    st = read(data)
    return st

def load_logo_trace():
    data = [0,0,0,2,4,6,4,2,0,-2,-4,-6,-4,-2,0,-1,-2,-1,0,-1,-2,-1,0,0,0,0,0]
    tr = Trace(data=data,header={'network':'UW','station':'PULSE','location':'--','channel':'LGO'})
    return tr
