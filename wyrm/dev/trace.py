...
class MLTrace(Trace):
    ...
    def __init__(self, data=np.array([], dtype=np.float32), header=None):
    ...
    # Set attributes/properties for indexing purposes
    def get_site(self):
        hdr = self.stats
    ...
    def get_fold_trace(self):
        ft = Trace(data=self.fold, header=deepcopy(self.stats))
        return ft
    ...
    def merge(self, trace, method='avg', fill_value=0., sanity_checks=True):
        """
        ...
        # Get relative sample positions of self and trace
        ...
        # Place current data into new indexing positions
        tmp_data_array[0, idx[0]:idx[1]+1] = self.data
        tmp_fold_array[0, idx[0]:idx[1]+1] = self.fold
        # Place new data into new indexing positions
    ...
    def split(self):
        ...
        wherein a Stream of MLTrace objects are returned and are accompanied
        by slices of the source MLTrace's fold information
        """
        # Not a masked array.
