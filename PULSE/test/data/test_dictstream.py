...
class TestDictStream(unittest.TestCase):
    ...
    def test_view(self):
        ds = self.md_ds.copy()
        ts = ds['UW.PAT2.--.EHZ'].stats.starttime
        te = ds['UW.NATEM.--.ENZ'].stats.endtime
        view = ds.view()
        ...
        # narrowed endtime
        view = ds.view(endtime=te)
        for _id in view.keys:
            if ds[_id].stats.endtime > te:
                self.assertAlmostEqual(view[_id].stats.endtime, te, delta = ds[_id].stats.delta)
            else:
                self.assertEqual(view[_id].stats.starttime, ds[_id].stats.starttime)

        # narrowed from both sides
        view = ds.view(starttime=ts, endtime=te)
        for _id in view.keys:
            if ds[_id].stats.starttime < ts:
                self.assertAlmostEqual(view[_id].stats.starttime, ts, delta=ds[_id].stats.delta)
            else:
            ...
            if ds[_id].stats.endtime > te:
                ...
                self.assertEqual(view[_id].stats.endtime, ds[_id].stats.endtime)
            
        # test keeping/dropping empty traces
        view_keep = ds.view(starttime=te+7, endtime=te + 10)
        view_drop = ds.view(starttime=te+7, endtime=te + 10, keep_empty_traces=False)
        # Sanity check that test setup results in some dropped, empty traces
        self.assertNotEqual(len(view_keep), len(view_drop))
        
        for _id in view_keep.keys:
            # If the trace view has data
            if view_keep[_id].count() > 0:
                # result should be identical
                self.assertEqual(view_keep[_id], view_drop[_id])
            # If the view is empty
            else:
                # the _id should not appear in view_drop
                self.assertNotIn(_id, view_drop.keys)
                # the starttime of the empty trace in view_keep should be the starttime
                self.assertEqual(view_keep[_id].stats.starttime, te+7)
                self.assertEqual(view_keep[_id].stats.starttime, view_keep[_id].stats.endtime)
                self.assertEqual(view_keep[_id].count(), 0)
    
    def test_normalize(self):
        # Test with defaults (max amplitude, localized)
        ds = self.sm_ds.copy().normalize()
        for ft in ds:
            # Assert local max give no greater than 1
            self.assertLessEqual(ft.data.max(), 1)
            # Assert local max gives no less than -1
            self.assertGreaterEqual(ft.data.min(), -1)
            # Assert local max gives a max amplitude of 1
            self.assertEqual(ft.max(), 1)
        # Test Aliases for max amplitude normalization
        for alias in ['max','minmax','peak']:
            dsA = self.sm_ds.copy().normalize(norm=alias)
            self.assertEqual(ds, dsA)

    def test_normalize_std(self):
        # Test with std normalization
        ds = self.md_ds.copy().normalize(norm='std')
        for ft in ds:
            ft0 = self.md_ds[ft.id]
            # Assert that normalized trace is a std-scaled equivalent
            np.testing.assert_array_equal(ft.data, ft0.data/ft0.std())
        
        # Test aliases for std normalization
        for alias in ['std','standard','sigma']:
            dsA = self.md_ds.copy().normalize(norm=alias)
            self.assertEqual(ds, dsA)
        
    def test_normalize_global(self):
        # Test with global_norm turned on
        # Determine which trace sets the global
        masetter = None
        mssetter = None
        maxamp = 0
        maxsig = 0
        for ft in self.sm_ds:
            ima = np.abs(ft.data).max()
            ims = ft.std()
            if ima > maxamp:
                maxamp = ima
                masetter = ft.id
        ...
        # Test global max amp
        ds = self.sm_ds.copy().normalize(global_norm=True)
        ...
        for ft in ds:
            ...
            # assert that 
            if ft.id == masetter:
                self.assertEqual(np.abs(ft.data).max(), 1)
            else:
                self.assertLessEqual(np.abs(ft.data).max(), 1)

         # Test global standard scaling
        ds = self.sm_ds.copy().normalize(norm='std', global_norm=True)

        for ft in ds:
            ft0 = self.sm_ds[ft.id]
            # Assert that normalized trace is a std-scaled equivalent
            np.testing.assert_array_equal(ft.data, ft0.data/maxsig)

    def test_normalize_scalar(self):

        ds = self.sm_ds.copy().normalize(norm=2)
        for ft in ds:
            ft0 = self.sm_ds[ft.id]
            np.testing.assert_equal(ft0.data/2, ft.data)

    def test_normalize_errors(self):
        with self.assertRaises(ValueError):
            self.sm_ds.copy().normalize(norm=-1)
        with self.assertRaises(ValueError):
            self.sm_ds.copy().normalize(norm=0.)
        with self.assertRaises(TypeError):
            self.sm_ds.copy().normalize(norm='a')
            # # assert that 
            # if ft.id == mssetter:
            #     self.assertEqual(ft.max()/ft0.max(), 1)
            # else:
            #     self.assertLess(np.abs(ft.data).max(), 1)
