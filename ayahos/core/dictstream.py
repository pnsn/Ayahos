"""
Module for handling Ayahos :class: `~ayahos.core.dictstream.DictStream` objects
...
def read_mltraces(data_files, obspy_read_kwargs={}, add_options={}):
    """
    Wrapper around :meth: `~ayahos.data.mltrace.MLTrace.read_mltrace`
    ...
    :param data_files: [str] file name of a {COMMON_NAME}_DATA.{EXTENSION}
                    ...
                    also see ayahos.data.mltrace.read_mltrace()
    ...
    :add_options: [dict] dictionary of keyword arguments to pass to
                    ayahos.data.DictStream.DictStream.__add__

    :: OUTPUT ::
    :return dst: [ayahos.data.DictStream.DictStream] assembled DictStream object
...
class DictStreamStats(AttribDict):
    """
    A class to contain metadata for a ayahos.core.DictStream.DictStream object
...
class DictStream(Stream):
    """
    Dictionary like object of multiple Ayahos :class: `~ayahos.core.mltrace.MLTrace` objects
    ...
    :type traces: list of :class: `~obspy.core.trace.Trace` (or children) objects, optional
        NOTE: all initial traces are converted into :class: `~ayahos.core.mltrace.MLTrace` objects
    :param header: initial dictionary of key-value pairs to pass to Ayahos
        :meth: `~ayahos.core.dictstream.DictStreamStats.__init__`, defaults to {}
    ...
    :type key_attr: str, optional
        Based on N.S.L.C.M.W from Ayahos :class: `~ayahos.core.mltrace.MLTraceStats`
        ...
        Supported values:
            'id' - N.S.L.C(.M.W) code (see :class: `~ayahos.core.mltrace.MLTrace`)
            'site' - N.S code
        ...
        also see :meth: `~ayahos.core.mltrace.MLTrace.get_id_element_dict`
    :param **options: key word argument gatherer that passes optional kwargs to
        :meth: `~ayahos.core.dictstream.DictStream.__add__` for merging input
    ...
    def __add__(self, other, **options):
        ...
        into a list and initialize a new :class: `~ayahos.core.dictstream.DictStream`
        object.
        ...
        :rtype: ayahos.core.dictstream.DictStream
        """
    ...
    def __str__(self):
        ...
        :class: `~ayahos.core.dictstream.DictStream`

        :return rstr: representative string
        :rtype rstr: str
        """
        rstr = 'ayahos.core.dictstream.DictStream'
        return rstr

    def __repr__(self, extended=False):
        """string representation of the contents of this
        Ayahos :class: `~ayahos.core.dictstream.DictStream` object
    ...
    #         **obspy_write_options):
    #     """
    #     Wrapper around the ayahos.data.mltrace.MLTrace.write() method
    #     that saves data as 
    ...
    def read(mltrace_mseed_files):
        ...
        also see ayahos.core.mltrace.MLTrace.write()
                 ayahos.core.mltrace.MLTrace.read()
        """
    ...
    def fnselect(self, fnstr, key_attr=None, ascopy=False):
        ...
        :rtype out: ayahos.core.dictstream.DictStream
        """
    ...
    def isin(self, iterable, key_attr=None, ascopy=False):
        ...
        :: OUTPUT ::
        :return out: [ayahos.data.DictStream.DictStream] subset view/copy
        """
        out = self.__class__(header=self.stats.copy(), key_attr = self.default_key_attr)
        matches = []
        for _e in iterable:
...
#         mltr = MLTrace(data=tr.data, header=hdr)
    ...
    #     """
    #     from ayahos.streaming.windowstream import ComponentStream
