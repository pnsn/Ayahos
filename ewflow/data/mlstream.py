...
from ewflow.util.pyew import wave2mltrace



# def read_mltraces(data_files, obspy_read_kwargs={}, add_options={}):
#     """
#     Wrapper around :meth:`~ewflow.data.mltrace.MLTrace.read_mltrace`
...
#                     also see ewflow.data.mltrace.read_mltrace()
...
#                     ewflow.data.MLStream.MLStream.__add__

#     :: OUTPUT ::
#     :return dst: [ewflow.data.MLStream.MLStream] assembled MLStream object
...
class MLStreamStats(AttribDict):
    """
    A class to contain metadata for a ewflow.data.MLStream.MLStream object
    ...
    def __init__(self, header={}):
        ...
        :class: `~ewflow.data.mlstream.MLStream` object
...
class MLStream(Stream):
    """
    Dictionary like object of multiple EWFlow :class: `~ewflow.data.mltrace.MLTrace` objects
    ...
    :type traces: list of :class: `~obspy.core.trace.Trace` (or children) objects, optional
        NOTE: all initial traces are converted into :class: `~ewflow.data.mltrace.MLTrace` objects
    :param header: initial dictionary of key-value pairs to pass to EWFlow
        :meth: `~ewflow.data.mlstream.MLStreamStats.__init__`, defaults to {}
    ...
    :type key_attr: str, optional
        Based on N.S.L.C.M.W from EWFlow :class: `~ewflow.data.mltrace.MLTraceStats`
        ...
        Supported values:
            'id' - N.S.L.C(.M.W) code (see :class: `~ewflow.data.mltrace.MLTrace`)
            'site' - N.S code
        ...
        also see :meth: `~ewflow.data.mltrace.MLTrace.get_id_element_dict`
    :param **options: key word argument gatherer that passes optional kwargs to
        :meth: `~ewflow.data.mlstream.MLStream.__add__` for merging input
    ...
    def __add__(self, other, **options):
        ...
        into a list and initialize a new :class: `~ewflow.data.mlstream.MLStream`
        object.
        ...
        :rtype: ewflow.data.mlstream.MLStream
        """
    ...
    def __str__(self):
        ...
        :class: `~ewflow.data.mlstream.MLStream`

        :return rstr: representative string
        :rtype rstr: str
        """
        rstr = 'ewflow.data.mlstream.MLStream'
        return rstr

    def __repr__(self, extended=False):
        """string representation of the contents of this
        EWFlow :class: `~ewflow.data.mlstream.MLStream` object
    ...
    #         **obspy_write_options):
    #     """
    #     Wrapper around the ewflow.data.mltrace.MLTrace.write() method
    #     that saves data as 
    ...
    def read(mltrace_mseed_files):
        ...
        also see ewflow.data.mltrace.MLTrace.write()
                 ewflow.data.mltrace.MLTrace.read()
        """
    ...
    def fnselect(self, fnstr, key_attr=None, ascopy=False):
        ...
        :rtype out: ewflow.data.mlstream.MLStream
        """
    ...
    def isin(self, iterable, key_attr=None, ascopy=False):
        ...
        :: OUTPUT ::
        :return out: [ewflow.data.MLStream.MLStream] subset view/copy
        """
        out = self.__class__(header=self.stats.copy(), key_attr = self.default_key_attr)
        matches = []
        for _e in iterable:
    ...
    def normalize_traces(self, norm_type ='peak'):
        ...
        norm_type and :meth: `~ewflow.data.mltrace.MLTrace.normalize`
    ...
    def prediction_trigger_report(self, thresh, exclude_list=None, **kwargs):
        """Wrapper around the :meth: `~ewflow.data.mltrace.MLTrace.prediction_trigger_report`
        ...
        :type thresh: float
        :param exclude_list: :meth: `~ewflow.data.mlstream.MLStream.notin` compliant strings
                        to exclude from trigger processing, defaults to None
        :type exclude_list: list of str, optional
        :param **kwargs: gatherer for key word arguments to pass to :meth: `~ewflow.data.mltrace.MLTrace.prediction_trigger_report`
